...
treatmentArrayListCreator_OldInt = function(M = 80, N = 20, iStrength = 1, univ = 1, sigmaMax = 0.1,
                                     thresholdMult = 10 ^ (-1), maxSteps = 10 ^ 4, tStep = 10 ^ (-2),
                                     intTime = 100, interSmplMult = 0.01, lambda = 10 ^ (-1),
                                     synthM = floor(M / 4), numSynthChrt = 10, numSample = 100,
                                     maxIters = 100){
  
  #  Uses old rules to determine which samples are interacting, and which are not. Creates a
  #  connected subgraph out of floor of half of the vertices, the rest don't interact at all,
  #  as in the old simulations.
  ...
  source("DOCNSDFListCreator_OldInt.r")
  source("embedGraph.r")
  ...
  countSuccessfulTests = function(coords){
    ...
    #  number of interacting samples
    numI = floor(M / 2)
    
    #  create connectivity graph using the graph type and mean degree parameters
    intGraph = conGraphCreator(graphType = graphType, meanDegree = meanDegree, v = numI)
    
    conGraph = embedGraph(M = M, intGraph = intGraph)
    
    testVecCreator = function(x = NULL){
      
      DOCNSDFList = DOCNSDFListCreator_OldInt(conGraph = conGraph, M = M, N = N, iStrength = iStrength,
  ...
  # 
  # clusterExport(cl = cl, list = ls(.GlobalEnv), envir = .GlobalEnv)
  # 
  # successfulTestCounts = parApply(cl = cl, coordArray, 1:3, countSuccessfulTests)
  # 
  # stopCluster(cl = cl)
...
} #  end treatmentArrayListCreator_OldInt function
