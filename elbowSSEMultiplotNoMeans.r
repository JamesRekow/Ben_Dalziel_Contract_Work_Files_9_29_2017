...
elbowSSEMultiplotNoMeans = function(elbowSSEDFLambda, elbowSSEDFM, elbowSSEDFNumSubgroups){
  ...
  SSEM100 = sapply(1:10, function(n) mean(subset(elbowSSEDFM, numCenters == n & M == 100)$SSE))
  SSEM160 = sapply(1:10, function(n) mean(subset(elbowSSEDFM, numCenters == n & M == 160)$SSE))
  
  meanNumCenters = rep(1:10, 3)
  meanSSE = c(SSEM40, SSEM100, SSEM160)
  meanM = as.factor(c(rep(40, 10), rep(100, 10), rep(160, 10)))
  
  meanDF2 = data.frame(numCenters = meanNumCenters, SSE = meanSSE, M = meanM)
  
  p2 = ggplot() +
    ggtitle("Effect of Number of Samples on SSE of K-Means",
            subtitle = "N = 20, numSubgroups = 6, lambda = 100, univ = 1") +
...
} #  end elbowSSEMultiplotNoMeans function
