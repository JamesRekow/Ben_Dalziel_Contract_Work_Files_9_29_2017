...
identifyNumSubgroupsTestSuite = function(iStrength = 1, sigmaMax = 0.1, thresholdMult = 10 ^ (-1),
                                         maxSteps = 10 ^ 4, tStep = 10 ^ (-2), intTime = 1,
                                         interSmplMult = 0.01, conGraph = NULL, rollRange = 0.025,
                                         numReplicates = 3){
  ...
  source("identifyNumSubgroupsTest.r")
  ...
  input = vector("list", 5)
  input[[1]] = c(40, 20, 2, 0, 1)
  input[[2]] = c(40, 20, 4, 100, 1)
  input[[3]] = c(40, 20, 4, 200, 1)
  input[[4]] = c(100, 20, 4, 100, 1)
  input[[5]] = c(100, 20, 4, 200, 0)
  
  #  create a matrix of all unique combinations of elements from Mvec, NVec, lambdaVec, and univVec
  inputMat = Reduce(rbind, input)
  
  performIdentifyNumSubgroupsTest = function(inputVec){
    ...
    meanPercentError = identifyNumSubgroupsTest(MVec = MVec, NVec = NVec, lambdaVec = lambdaVec,
                                                numSubgroupsVec = numSubgroupsVec, iStrength = iStrength,
                                                univVec = univVec, sigmaMax = sigmaMax,
                                                thresholdMult = thresholdMult, maxSteps = maxSteps,
                                                tStep = tStep, intTime = intTime,
                                                interSmplMult = interSmplMult, conGraph = conGraph,
                                                rollRange = rollRange, numReplicates = numReplicates)[[6]]
    
    return(meanPercentError)
    
  } #  end performIdentifyNumSubgroupsTest function
  
  meanPercentErrorVec = apply(inputMat, 1, performIdentifyNumSubgroupsTest)
...
} #  end identifyNumSubgroupsTestSuite function
