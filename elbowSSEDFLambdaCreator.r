...
elbowSSEDFLambdaCreator  = function(MVec = NULL, numSubgroupsVec = NULL, lambdaVec = NULL, numReplicates = 10){
  ...
  createSSEReplicate = function(replicateID){
    ...
    len = length(SSE1)
    lambda1 = rep(lambdaVec[1], len)
    lambda2 = rep(lambdaVec[2], len)
    lambda3 = rep(lambdaVec[3], len)
    
    lambda = as.factor(c(lambda1, lambda2, lambda3))
    ...
    SSEDF = data.frame(runID = runID, numCenters = numCenters, SSE = SSE, lambda = lambda)
...
} #  end  elbowSSEDFLambdaCreator function
