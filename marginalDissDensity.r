...
marginalDissDensity = function(M = 40, numSubgroups = 2, N = 20, iStrength = 1, univ = 1, sigmaMax = 0.1,
  ...
  p1 = ggplot(doc, aes(x = over, y = diss, col = sameSubgroup)) + geom_point() + ggtitle("DOC Points")
  ...
  ggplotSubgroupDissDens = function(over){
    ...
    densPlot = ggplot(similarOverDOC, aes(diss, col = sameSubgroup)) +
                 geom_density() +
                 xlab("") +
                 ylab("") +
                 xlim(0, 0.4)
  ...
  #  create ggplot object arranging both density plots into a single figure
  p2 = grid.arrange(arrangeGrob(meanOverDissDensPlot, top = paste("Mean Overlap: ", round(meanOver, 2))),
                    arrangeGrob(highOverDissDensPlot, top = paste("High Overlap: ", highOver)),
                    ncol = 2, left = "Density", bottom = "Dissimilarity", top = titleText)
  
  #  plot DOC ggplot object in new graphics device
  dev.new()
  plot(p1)
  
  #  plot density ggplot object in new graphics device
  dev.new()
  plot(p2)
