...
elbowSSETest  = function(M = 40, numSubgroups = 2, N = 20, iStrength = 1, univ = 1,
  ...
  #  compute SSE of clusters for kmeans results using each number of centers in centersVec
  SSEVec = sapply(1:length(centersVec), function(n) computeClusterSSE(inputMat = infoMat,
                                                                      centers = centersVec[n]))
  
  return(SSEVec)
  
} #  end  elbowSSETest  function
