...
computeInfoWeightMat_Exp_Weight = function(abdList, rollRange = 0.025){
  ...
  computeInfoWeight = function(docPoint){
    ...
    #  only share a single species (default 0, nonzero worked poorly)
    if(diss < 0.005){
      if(over < 0.7){
        return(0)
      } #  end if
    ...
    #  NOTE: Would it be better to just check if the length of localDiss is zero?
    
    #  to prevent NaNs caused by localDiss vectors with only a single value (leads to division by zero)
    #  also, lone points provide no information
    validInfoWeight = {localDissMax > localDissMid} && {localDissMin < localDissMid}
    ...
    if(validInfoWeight){
      ...
      # val = diss - localDissMid
      # 
      # infoWeight = sign(val) * sqrt(abs(val))
  ...
  #  create matrix of info weights (-1 default diagonal value)
