...
identifySubgroupsVer1ErrorCheck_Exp_Weight = function(MVec = NULL, NVec = NULL, lambdaVec = NULL,
  ...
  percentErrorProducer = function(inputVec){
    ...
    #  if number of clusters for treatments is known, set it equal to the number of subgroups
    if(numClustKnown){
      numClust = numSubgroups
    } #  end if
    
    #  compute percent error of algorithm's subgroup classification
    percentError = identifySubgroupsVer1Test_Exp_Weight(M = M, numSubgroups = numSubgroups, N = N,
                                                        iStrength = iStrength, univ = univ,
                                                        sigmaMax = sigmaMax, thresholdMult = thresholdMult,
                                                        maxSteps = maxSteps, tStep = tStep,
                                                        intTime = intTime, interSmplMult = interSmplMult,
                                                        lambda = lambda, returnParams = FALSE,
                                                        conGraph = NULL,  rollRange = rollRange,
                                                        numClust = numClust)
...
} #  end identifySubgroupsVer1ErrorCheck_Exp_Weight function
